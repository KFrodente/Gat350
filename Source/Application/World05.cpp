...
namespace nc
{
    bool World05::Initialize()
    {
...

        {
            auto actor = CREATE_CLASS(Actor);
            actor->name = "camera1";
            actor->transform.position = glm::vec3{ 0, 0, 1 };
...

        {
            auto actor = CREATE_CLASS(Actor);
            actor->name = "light1";
            actor->transform.position = glm::vec3{ 7, 3, 3 };
...

        {
            auto actor = CREATE_CLASS(Actor);
            actor->name = "light2";
            actor->transform.position = glm::vec3{ 1, 3, 3 };
            auto lightComponent = CREATE_CLASS(LightComponent);
            lightComponent->type = LightComponent::eType::Point;
            lightComponent->color = glm::rgbColor(glm::vec3{ randomf() * 360, 1, 1 });
            lightComponent->intensity = 1;
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            actor->transform.scale = glm::vec3{ randomf(0.5, 4), randomf(0.5, 4), 1 };
            actor->Initialize();
            m_scene->Add(std::move(actor));
        }
    ...
    void World05::Update(float dt)
    {
        ...
        if (material)
        {
            ...
            ImGui::DragFloat("IOR", &m_refraction, 0.01f, 1, 10);
        ...
        //setting up reflection ogre
        actor = m_scene->GetActorByName<Actor>("actor2");
        material = actor->GetComponent<ModelComponent>()->material;

        material->ProcessGui();
        material->Bind();

        //setting up phong ogre
